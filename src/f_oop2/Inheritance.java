...
public class Inheritance {
	...
	public static void main(String[] args) {
		 ...
		 *	 - 형반환이 인스턴스에 영향을 주진 않는다.
		 *	 - 부모타입에서 자식타입으로 형변환시 instanceof 연산자로 인스턴스를 확인하는 것이 안전하다.
		 *	 - 보마와 자식에 동일한 메서드가 있는 경우 자식의 메서드를 사용한다.
...
//		System.out.println(t);
		...
		//문제2. Time 클래스의 시분초 셋팅시 초과되는 부분을 고려한 로직을 만들어 주세요.
		t.setSecond(100000);
		System.out.println(t);
		 ...
		 * - remove(Object) : 저장되어 있는 객체를 제거한다. 제거에 성공하면 true 실패하면 false를 반환한다.
		 * - isEmpty() : 비어있는지 검사한다. 비어있으면 true 비어있지않으면 false를 반환한다.
		 * - get(int) : 지정된 위치에 객체를 반환한다. 반환타입은 Object 이므로 형변환이 필요하다.
