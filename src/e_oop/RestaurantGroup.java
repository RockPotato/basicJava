...
public class RestaurantGroup 
{
	...
	static {
		...
		menus = new String[restaurants.length][];
		menus[0] = new String[] { "김밥", "라면", "떡볶이", "오뎅", "순대" };
		menus[1] = new String[] { "짜장면","짬뽕","볶음밥","탕수육"};
		menus[2] = new String[] { "불고기버거", "와퍼", "상하기버거", "콜라", "사이다", "감자튀김" };
	}
	
	RestaurantGroup(String name) //생성자는 인스턴스변수를 초기화하거나 인스턴스화 할 때 필요한 작업을 위해 사용한다.
	{
		//생성자를 통한 초기화(3번째)
		if(name.equals("분식집"))
		{
	...
	RestaurantGroup()
	{
		this("버거집"); //생성자에서 생성자를 호출 할때는 반드시 this()를 사용
	}
	//메뉴판 좀 주세요.
	String[] getMenu()
	{
