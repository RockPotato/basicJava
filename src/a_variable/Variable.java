...
/*		이것은
		 ...
		 * - long		(8 byte) : -9223372036854775808 ~ 9223372036854775807(-2^63 ~ 2^63 -1, 약 -900경 ~ 약 900경)
		 * - float		(4 byte) : 1.4 * 10^-45 ~ 3.4 * 10^38(양의 범위) 7자리의 정확성
		 * - double		(8 byte) : 4.9 * 10^-324 ~ 1.8 * 10^308(양의 범위) 15자리의 정확성
		 * 
		 *	5.형변환
		 * - 데이터의 타입을 다른 타입으로 변환하는 것
		 * - 형변환하고자 하는 데이터 앞에 변환하고자하는 타입을 괄호와 함게 붙여준다.
		 * - 형변환하고자 하는 데이터의 표현범위보다 크기가 큰 표현범위로의 변환은 생량이 가능하다.
		 * 
		 *	6. 참조형 변수
		 ...
		 * 
		 *	7. 상수와 리터럴
		 * - 상수 : 값을 한번 저장하면 변경할 수 없는 저장공간 (final 사용)
		 * - 리터럴 : 그 자체로 값을 의미하는 것
		 */
		boolean bool;
