...
public class Exam05 {
	...
	public static void main(String[] args) {
		...
		하시오.
			...
			f. int[] arr[] = new int[3][];
		답 : d, =뒤에 값을 선언을 두번하였다.
			e, 선언의 방식이 잘못 되었다. 5를 없애거나 c의 방법으로 선언해야한다.
		...
		답 :		int tmp=0;
			...
			if(coin[i]<coinNum)
			{
				tmp=coin[i]*coinUnit[i];
				coin[i]=0;
				money-=tmp;
			}
			else
			{
				tmp=coinNum*coinUnit[i];
				coin[i]-=coinNum;
				money-=tmp;
			} 
		...
		2**
		4****
		답 : (1)	count[answer[i]-1]++;
			(2) System.out.print(counter[i]);
		...
		**
		...
		답 : if(ch-'a'>=0&&ch-'z'<=25)
			{
				result+=(char)abcCode[ch-'a'];
			}
			else if(ch-'0'>=0&&9>=ch-'9')
			{
				result+=(char)numCode[ch-'0'];
			}
		...
		답 : for(int j=0;j<question.length;j++)
			{
				int temp=(int)(Math.random()*question.length);
				char temp1 =question[0];
				
				question[0]=question[temp];
				question[temp]=temp1;
