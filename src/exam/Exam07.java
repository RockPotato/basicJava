	...
	public static void main(String[] args) {
		 ...
		 * [7-1] 섯다카드 20장을 포함하는 섯다카드 한 벌(SutdaDeck클래스)을 정의한 것이다. 섯
			...
			답 : void shuffle()
				...
				{
					SutdaCard pickSutda = cards[index];
					return pickSutda; 
				}
				SutdaCard pick()
				{
					int temp=(int) (Math.random()*CARD_NUM);
					SutdaCard pickSutda = cards[temp];
					return pickSutda;
				}
			[7-3] 오버라이딩의 정의와 필요성에 대해서 설명하시오.
			 답 : 상속을 받은 클래스의 메서드와 이름과 매개변수의 수와 타입이 같아야 하며 조상 클래스의 기능으로부터
			 	추가하여 사용하거나 기능을 달리하기 위하여 사용한다.
			...
			[7-5] 다음의 코드는 컴파일하면 에러가 발생한다. 그 이유를 설명하고 에러를 수정하기
				...
				답 : 조상 클래스(Product)에 생성자가 있어서 하위 클래스(Tv 클래스)의 생성자에 아무 것도 없으면
						하위 클래스의 변수가 조상 클래스의 멤버에 접근할 수도 있기 때문에 조상 클래스의 생성자를 
						정의 하여야 한다.
						Tv(){ //중괄호 안에 super(int price)를 정의 하여야 한다.
						
				[7-6] 자손 클래스의 생성자에서 조상 클래스의 생성자를 호출해야하는 이유는 무엇인
					가?
					답 : 자손 클래스의 변수가 조상 클래스의 변수를 사용할 수 있기 때문에
				...
				CH:10
				VOL:20
				답 : public void setChannel(int i) {
						if(i >= MIN_CHANNEL&&i<=MAX_CHANNEL)
						{
							this.channel=i;
						}
						else
					...
					public void setVolume(int i) {
						if(i>=MIN_VOLUME&&i<=MAX_VOLUME)
						{
							this.volume=i;
						}
						else
						{
							System.out.println("wrong volume try again");
						}
					}
				...
				[7-13] Math클래스의 생성자는 접근 제어자가 private이다. 그 이유는 무엇인가?
					답 : Math 클래스 이외에 클래스에서 사용하는 것을 방지하기 위해
				...
				답 : class Marine extends Unit{ // 보병
					void stimPack() {  스팀팩을 사용한다.}
					}
				...
				노래를 합니다.
				그림을 그립니다.
				답 : private static void action(Robot r) {
				...
				답 : class Buyer {
						...
						void add(Product p) {
							if(i>=cart.length)
							{
								Product[] moreCart=new Product[cart.length*2];
				...
				답 : static double sumArea(Shape[] arr) {
						double sum = 0 ;
						for(int i=0;i<arr.length;i++)
						{
							sum+=arr[i].calcArea();
				...
				[실행결과]
				100
				답 : Outer.Inner o =new Outer().new Inner();
				...
				[실행결과]
				200		
				답 : System.out.println(new Outer.Inner().iv);
				...
				답 : (1) value
					(2) this.value
					(3) new Outer().value
					(4) Inner inner =new Outer().new Inner();
				...
				답 : 지역 클래스를 포함한 메서드가 종료가 되어도 지역 클래스가 참조될 수 있기 때문에 
					메서드의 내용을 변경할 수 없도록 상수만 접근 가능하다.
		 */
