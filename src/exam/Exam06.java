	...
	public static void main(String[] args) {
			...
			[6-2] 문제6-1에서 정의한 SutdaCard클래스에 두 개의 생성자와 info()를 추가해서 실행
				...
				답 : int num;
					...
					SutdaCard(int num1,boolean isKwang1) 
					{
						this.num=num1;
						this.isKwang=isKwang1;
					}
					SutdaCard() {
						this(1, true);
					}
			...
			getTotal()과 getAverage()를 추가하시오.
				...
				답 : 	String name;
						...
						float getAverage()
						{
							float avr=getTotal()/3f;
							avr=(int)((avr+0.05f)*10)/10f;
							return avr;
						} 
			...
			[6-5] 다음과 같은 실행결과를 얻도록 Student클래스에 생성자와 info()를 추가하시오.
				...
				답 : String name;
					...
					public String info() {
						...
						float avr=sum/3f;
						avr=(int)((avr+0.05f)*10)/10f;
						info=name+","+ban+","+no+","+kor+","+eng+","+math+","+sum+","+avr;
						return info;
					}
			...
			드로 정의하시오.
				...
				답 : double getDistance(int x1, int y1) 
					{
						int a = x1 - x;    // 선 a의 길이
					    int b = y1 - y;    // 선 b의 길이
					    double c = Math.sqrt((a * a) + (b * b));
			...
			(단, 모든 병사의 공격력과 방어력은 같아야 한다.)
				...
				답 : weapon 과 armor에 붙여야 한다. 모든 병사의 공격력과 방어력은 같아야해서
			...
			이유를 설명하시오.
				...
				답 : A, iv 인스턴스변수이고 인스턴스변수를 사용하려면 인스턴스화 필요
					B, iv 인스턴스변수이고 인스턴스변수를 사용하려면 인스턴스화 필요 
					D, instanceMethod1가 static이 아니기 때문에
			...
			[6-20] 다음과 같이 정의된 메서드를 작성하고 테스트하시오.
				...
				[1, 2, 3, 4, 5, 6, 7, 8, 9]
				[4, 6, 8, 3, 2, 9, 7, 1, 5]
				답 : static int[] shuffle(int[] original) 
					{
						for(int i=0;i<original.length;i++)
						{
							int ran=(int) (Math.random()*original.length);
							int temp=original[i];
							original[i]=original[ran];
							original[ran]=temp;
						}
						return original;
					}
